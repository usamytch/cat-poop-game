...
- **Коридорный режим**: боковые полосы (col 0 и col GRID_COLS-1) **полностью свободны**; вертикальные стены (cols 7, 14, 21) начинаются с row=0 — чтобы верхняя полоса не была открытым карманом (хозяин 36×36px перекрывает row=0 и row=1 одновременно)

...
- Срочность растёт по формуле `urgeRate / 60 * (1 + (level - 1) * 0.08)` с мягким капом.
...
- Любая правка генерации уровня требует `tests/level.test.js`, `tests/grid.test.js` и проверки подвала.
